...
class Kriteria extends CI_Controller
{
	...
	function save_prioritas()
	{
		...
		foreach ($input['kriteria'] as $key => $value) {
			...
			$cek = $this->db->query('SELECT * from prioritas_kriteria where kriteria = ?', array($input['kriteria'][$key]))->row();

			if ($cek != null) {

				$query = $this->db->query('UPDATE prioritas_kriteria set prioritas = ? where kriteria = ?', array($input['prioritas'][$key], $input['kriteria'][$key]));
			} else {
				$query = $this->db->query('INSERT into prioritas_kriteria(kriteria, prioritas) values("' . $input['kriteria'][$key] . '","' . $input['prioritas'][$key] . '")');
			}
		}
